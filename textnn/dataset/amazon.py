...
def amazon_star_rating_generator(dataset_file: Path) -> Iterable[Tuple[str, int]]:
    """
    Generate a text to star-rating tuples from a review tsv file.
    :param dataset_file: the tsv file containing the reviews
    ...
    return (get_text_and_label(line) for line in read_lines(file_path=dataset_file, ignore_first_n_lines=1))


def amazon_binary_review_generator(dataset_file: Path, label_3_stars_as=None,
